...
Defaults.MAX_MAIN_ADDRESS_GAP = 12;

// TODO: should allow different gap sizes for external/internal chains
Defaults.SCAN_ADDRESS_GAP = Defaults.MAX_MAIN_ADDRESS_GAP + 12;
...
Defaults.RateLimit = {
  createWallet: {
    ...
    delayAfter: 12, // begin slowing down responses after the 12-s request
    delayMs: 3000, // slow down subsequent responses by 3 seconds per request
    max: 18, // start blocking after 18 request
    message: 'Too many wallets created from this IP, please try again after an hour',
  },
  estimateFee: {
   windowMs: 60 * 10 *1000, // 10 min window
    delayAfter: 18, // begin slowing down responses after the 18-s request
    delayMs: 500, // slow down subsequent responses by 1 seconds per request

    max: 100, // start blocking after 100 request
