  ...
  function withSignature(cb) {
    ...
    server.storage.fetchCopayerLookup(opts.copayerId, function(err, copayer) {
      ...
      if (!copayer.isSupportStaff) {
        // debugger; // ??only login??
        var isValid = !!server._getSigningKey(opts.message, opts.signature, copayer.requestPubKeys, 'safe');
        if (!isValid)
...
WalletService.prototype.createWallet = function(opts, cb) {
  ...
  if (!checkRequired(opts, ['name', 'm', 'n', 'pubKey', 'coin'], cb)) debugger; // return;
...
// debugger;
  if (!Utils.checkValueInCollection(opts.coin, Constants.COINS))
    return cb(new ClientError('Invalid coin1'));
...
// debugger;
  if (opts.coin && opts.coin == 'btc') {
    try {
      pubKey = new require('bitcore-lib').PublicKey.fromString(opts.pubKey);
    } catch (ex) {
      return cb(new ClientError('Invalid public btc key'));
    };
  } else if (opts.coin && opts.coin == 'bch') {
    try {
      pubKey = new require('bitcore-lib').PublicKey.fromString(opts.pubKey);
    } catch (ex) {
      return cb(new ClientError('Invalid public bch key'));
    };
  } else if (opts.coin && opts.coin == 'safe') {
    try {
      pubKey = new require('bitcore-lib-safe-mini').PublicKey.fromString(opts.pubKey);
    } catch (ex) {
      return cb(new ClientError('Invalid public safe key'));
    };
  } else if (opts.coin && opts.coin == 'btcz') {
    try {
      pubKey = new require('bitcore-lib-btcz-mini').PublicKey.fromString(opts.pubKey);
    } catch (ex) {
      return cb(new ClientError('Invalid public btcz key'));
    };
  } else if (opts.coin && opts.coin == 'zcl') {
    try {
      pubKey = new require('bitcore-lib-zcl-mini').PublicKey.fromString(opts.pubKey);
    } catch (ex) {
      return cb(new ClientError('Invalid public zcl key'));
    };
  } else if (opts.coin && opts.coin == 'anon') {
    try {
      pubKey = new require('bitcore-lib-anon-mini').PublicKey.fromString(opts.pubKey);
    } catch (ex) {
      return cb(new ClientError('Invalid public anon key'));
    };
  } else if (opts.coin && opts.coin == 'zel') {
    try {
      pubKey = new require('bitcore-lib-zel-mini').PublicKey.fromString(opts.pubKey);
    } catch (ex) {
      return cb(new ClientError('Invalid public zel key'));
    };
  } else if (opts.coin && opts.coin == 'rvn') {
    try {
      pubKey = new require('bitcore-lib-rvn-mini').PublicKey.fromString(opts.pubKey);
    } catch (ex) {
      return cb(new ClientError('Invalid public rvn key'));
    };
  } else if (opts.coin && opts.coin == 'ltc') {
    try {
      pubKey = new require('bitcore-lib-ltc-mini').PublicKey.fromString(opts.pubKey);
    } catch (ex) {
      return cb(new ClientError('Invalid public ltc key'));
    };
  } else {
    debugger;
  }
  ...
  async.series([
    ...
    function(acb) {
      ...
      self.storage.storeWallet(wallet, function(err) {
        self.logi('Wallet created', wallet.id, opts.coin + '-' + opts.network);
        newWallet = wallet;
        return acb(err);
      });
    }
 ...
 * @param xPubKey
 */
WalletService.prototype._verifyRequestPubKey = function(requestPubKey, signature, xPubKey, coin) {
// debugger;
  var pub = (new require('bitcore-lib').HDPublicKey(xPubKey)).deriveChild(Constants.PATHS.REQUEST_KEY_AUTH).publicKey;
...
WalletService.prototype.addAccess = function(opts, cb) {
  ...
  self.storage.fetchCopayerLookup(opts.copayerId, function(err, copayer) {
    ...
    self.storage.fetchWallet(copayer.walletId, function(err, wallet) {
      ...
      // debugger;
      if (!self._verifyRequestPubKey(opts.requestPubKey, opts.signature, xPubKey, opts.coin || wallet.coin)) {
...
WalletService.prototype.joinWallet = function(opts, cb) {
  ...
  opts.coin = opts.coin || Defaults.COIN;
  if (!Utils.checkValueInCollection(opts.coin, Constants.COINS))
    return cb(new ClientError('Invalid coin2'));

  var xPubKey;
  var invalk = 'Invalid extended public key ' + opts.coin;
  if (opts.coin && opts.coin == 'btc') {
    try {
      xPubKey = require('bitcore-lib').HDPublicKey(opts.xPubKey);
    } catch (ex) {
      return cb(new ClientError(invalk));
    }
  } else if (opts.coin && opts.coin == 'bch') {
    try {
      xPubKey = require('bitcore-lib').HDPublicKey(opts.xPubKey);
    ...
    try {
      xPubKey = require('bitcore-lib-safe-mini').HDPublicKey(opts.xPubKey);
    } catch (ex) {
      return cb(new ClientError(invalk));
    }
  } else if (opts.coin && opts.coin == 'btcz') {
    try {
      xPubKey = require('bitcore-lib-btcz-mini').HDPublicKey(opts.xPubKey);
    } catch (ex) {
      return cb(new ClientError(invalk));
    }
  } else if (opts.coin && opts.coin == 'zcl') {
    try {
      xPubKey = require('bitcore-lib-zcl-mini').HDPublicKey(opts.xPubKey);
    } catch (ex) {
      return cb(new ClientError(invalk));
    }
  } else if (opts.coin && opts.coin == 'anon') {
    try {
      xPubKey = require('bitcore-lib-anon-mini').HDPublicKey(opts.xPubKey);
    } catch (ex) {
      return cb(new ClientError(invalk));
    }
  } else if (opts.coin && opts.coin == 'zel') {
    try {
      xPubKey = require('bitcore-lib-zel-mini').HDPublicKey(opts.xPubKey);
    } catch (ex) {
      return cb(new ClientError(invalk));
    }
  } else if (opts.coin && opts.coin == 'rvn') {
    try {
      xPubKey = require('bitcore-lib-rvn-mini').HDPublicKey(opts.xPubKey);
    } catch (ex) {
      return cb(new ClientError(invalk));
    }
  } else if (opts.coin && opts.coin == 'ltc') {
    try {
      xPubKey = require('bitcore-lib-ltc-mini').HDPublicKey(opts.xPubKey);
    } catch (ex) {
      return cb(new ClientError(invalk));
    }
  } else {
...
WalletService.prototype._getUtxos = function(coin, addresses, cb) {
  ...
  if (coin == 'btc') {
    ...
    BA = require('bitcore-lib-safe-mini').Address;
  } else if (coin == 'btcz') {
  ...
  var networkName = BA(addresses[0]).toObject().network;
...
WalletService.prototype._getUtxosForCurrentWallet = function(opts, cb) {
  ...
  async.series([
    ...
    function(next) {
      ...
      if (Utils.getAddressCoin(addressStrs[0]) == 'bch', 'bch') 
        return next();
...
WalletService.prototype.getUtxos = function(opts, cb) {
  ...
  if (opts.coin) {
    if (!Utils.checkValueInCollection(opts.coin, Constants.COINS))
      return cb(new ClientError('Invalid coin3'));
  }
  ...
  if (_.isUndefined(opts.addresses)) {
    ...
    self._getUtxos(Utils.getAddressCoin(opts.addresses[0], opts.coin), opts.addresses, cb);
  }
};
...
WalletService.prototype.getBalance = function(opts, cb, i) {
  ...
  if (opts.coin) {
    if (!Utils.checkValueInCollection(opts.coin, Constants.COINS))
      return cb(new ClientError('Invalid coin4'));
  }
...
//  var bc = self._getBlockchainExplorer(coin, network);
  var bc = self._getBlockchainExplorer(coin == 'btc' ? 'btc' : 'bch', network);
  if (!bc) return cb(new Error('Could not get blockchain explorer instance'));
...
WalletService.prototype.getFeeLevels = function(opts, cb) {
  ...
  opts.coin = opts.coin || Defaults.COIN;
  if (!Utils.checkValueInCollection(opts.coin, Constants.COINS))
    return cb(new ClientError('Invalid coin5'));
...
WalletService.prototype._checkTx = function(txp) {
  ...
  var Ber;
  if (typeof coin === 'undefined' && txp.coin) var coin = txp.coin; 
  if (coin == 'btc') {
    Ber = require('bitcore-lib').errors.Transaction
  } else if (coin == 'bch') {
    Ber = require('bitcore-lib-cash').errors.Transaction
  } else if (coin == 'safe') {
    Ber = require('bitcore-lib').errors.Transaction
  } else if (coin == 'btcz') {
    Ber = require('bitcore-lib').errors.Transaction
  ...
  if (bitcoreError instanceof Ber.FeeError)
    return Errors.INSUFFICIENT_FUNDS_FOR_FEE;

  if (bitcoreError instanceof Ber.DustOutputs)
  ...
  function select(utxos, cb) {
    ...
    _.each(smallInputs, function(input, i) {
      ...
      if (netTotal >= txpAmount) {
        ...
        var BtxD;
       // debugger;
        if (typeof coin === 'undefined' && input.address) {
          try {
            new require('bitcore-lib').Address(input.address);
            var coin = 'btc';
          } catch (e) {
            try {
              new require('bitcore-lib-cash').Address(input.address);
              var coin = 'bch';
            } catch (e) {
              try {
                new require('bitcore-lib-safe-mini').Address(input.address);
                var coin = 'safe';
              } catch (e) {
                try {
                  new require('bitcore-lib-ltc-mini').Address(input.address);
                  var coin = 'ltc';
                } catch (e) {
                  try {
                    new require('bitcore-lib-rvn-mini').Address(input.address);
                    var coin = 'rvn';
                  } catch (e) {
                    try {
                      new require('bitcore-lib-btcz-mini').Address(input.address);
                      var coin = 'btcz';
                    } catch (e) {
                      try {
                        new require('bitcore-lib-anon-mini').Address(input.address);
                        var coin = 'anon';
                      } catch (e) {
                        new require('bitcore-lib-zcl-mini').Address(input.address);
                        var coin = 'zcl';
                      }
                    }
                  }
        ...
        if (coin == 'btc') {
          BtxD = require('bitcore-lib').errors.Transaction.DUST_AMOUNT
        } else if (coin == 'bch') {
          BtxD = require('bitcore-lib-cash').errors.Transaction.DUST_AMOUNT
        } else if (coin == 'safe') {
          BtxD = require('bitcore-lib').errors.Transaction.DUST_AMOUNT
        } else if (coin == 'btcz') {
          BtxD = require('bitcore-lib').errors.Transaction.DUST_AMOUNT
        ...
        var dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, BtxD);
...
WalletService.prototype._validateOutputs = function(opts, wallet, cb) {
  ...
  if (typeof coin === 'undefined' && wallet.coin) var coin = wallet.coin;
  if (coin == 'btc') {
    A = require('bitcore-lib').Address;
    TD = require('bitcore-lib').Transaction.DUST_AMOUNT
  } else if (coin == 'bch') {
    A = require('bitcore-lib-cash').Address;
    TD = require('bitcore-lib-cash').Transaction.DUST_AMOUNT
  } else if (coin == 'safe') {
    A = require('bitcore-lib-safe-mini').Address;
    TD = require('bitcore-lib-safe-mini').Transaction.DUST_AMOUNT
  } else if (coin == 'btcz') {
    A = require('bitcore-lib-btcz-mini').Address;
    TD = require('bitcore-lib-btcz-mini').Transaction.DUST_AMOUNT
  } else if (coin == 'zcl') {
    A = require('bitcore-lib-zcl-mini').Address;
    TD = require('bitcore-lib-zcl-mini').Transaction.DUST_AMOUNT
  } else if (coin == 'anon') {
    A = require('bitcore-lib-anon-mini').Address;
    TD = require('bitcore-lib-anon-mini').Transaction.DUST_AMOUNT
  } else if (coin == 'zel') {
    A = require('bitcore-lib-zel-mini').Address;
    TD = require('bitcore-lib-zel-mini').Transaction.DUST_AMOUNT
  } else if (coin == 'rvn') {
    A = require('bitcore-lib-rvn-mini').Address;
    TD = require('bitcore-lib-rvn-mini').Transaction.DUST_AMOUNT
  } else if (coin == 'ltc') {
    A = require('bitcore-lib-ltc-mini').Address;
    TD = require('bitcore-lib-ltc-mini').Transaction.DUST_AMOUNT
  }
  var dustThreshold = Math.max(Defaults.MIN_OUTPUT_AMOUNT, TD);
  ...
  self._runLocked(cb, function(cb) {
    self.getWallet({}, function(err, wallet) {
      ...
      self.storage.fetchTx(self.walletId, opts.txProposalId, function(err, txp) {
        ...
        // debugger;
        var signingKey = self._getSigningKey(raw, opts.proposalSignature, copayer.requestPubKeys, txp.coin);
        if (!signingKey) {
...
WalletService.prototype.broadcastRawTx = function(opts, cb) {
  ...
  opts.coin = opts.coin || Defaults.COIN;
  if (!Utils.checkValueInCollection(opts.coin, Constants.COINS))
    return cb(new ClientError('Invalid coin6'));
